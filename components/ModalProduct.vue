...
const { storeProduct, updateProduct } = useProductsStore();
...
<template>
    <UModal>
        <UCard>
            <div v-if="modalId === 1" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 md:gap-x-20 space-y-6 md:space-y-0">
                    <div>
                ...
                <div class="flex items-center justify-end space-x-2">
                    <button @click.prevent="closeModal()" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded border border-gray-200 text-sm font-medium px-4 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 min-w-[70px]">Cancel</button>
                    <button @click.prevent="submit()" type="button" class="text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800 min-w-[70px]">Save</button>
                </div>
            </div>
            <div v-if="modalId === 2" class="p-6 space-y-6">
                <h3 class="text-lg font-bold text-gray-900 lg:text-2xl dark:text-white">
                    Update Product
                </h3>
                <div>
                ...
                <div class="flex items-center justify-end space-x-2">
                    <button @click.prevent="closeModal()" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded border border-gray-200 text-sm font-medium px-4 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 min-w-[70px]">Cancel</button>
                    <button @click.prevent="update()" type="button" class="text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800 min-w-[70px]">Save</button>
                </div>
            </div>
        </UCard>
    </UModal>
